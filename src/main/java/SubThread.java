...
class FindFileVisitor extends SimpleFileVisitor<Path> {
    ...
    public void insertErrorStatus(String path, int projectId, int status) throws SQLException {
        this.con.preSt = this.con.con.prepareStatement("insert into API_sequence(projectId, path, disk, status) values(?,?,?,?)");
...
//                            System.out.println(diskInDB);
                        ...
                        String deleteSql = "delete from API_sequence where projectId = " + Integer.toString(projectId);
                        try {
